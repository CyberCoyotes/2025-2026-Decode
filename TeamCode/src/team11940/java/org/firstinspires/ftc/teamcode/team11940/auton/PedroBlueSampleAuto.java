...
/**
 * Pedro Pathing Sample Autonomous - Blue Alliance
 * Team 11940
 *
 ...
 */
@Autonomous(name = "Blue: Pedro Sample Auto", group = "Autonomous")
public class PedroBlueSampleAuto extends LinearOpMode {
     ...
     * - X: positive = right (from blue alliance wall)
     * - Y: positive = forward (toward red alliance)
     * - Heading: 0° = facing forward (toward red alliance)
     * ======================================== */

    // Starting position - Blue alliance, right side
    private final Pose START_POSE = new Pose(-12, -63, Math.toRadians(90));

    // Specimen scoring position (at submersible)
    private final Pose SPECIMEN_SCORE_POSE = new Pose(-6, -36, Math.toRadians(90));

    // Sample collection positions
    private final Pose SAMPLE_1_POSE = new Pose(-48, -40, Math.toRadians(90));
    private final Pose SAMPLE_2_POSE = new Pose(-58, -40, Math.toRadians(90));

    // High basket scoring position
    private final Pose HIGH_BASKET_POSE = new Pose(-52, -52, Math.toRadians(135));

    // Observation zone parking position
    private final Pose PARK_POSE = new Pose(-12, -12, Math.toRadians(90));
